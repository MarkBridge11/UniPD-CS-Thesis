...
%L'obiettivo di questa attività di stage era quello di inserirmi all'interno del progetto, più precisamente nello sviluppo del lato back-end sviluppando un'API REST e le tabelle del database su cui poi l'API poggiava. Le tabelle dovevano inoltre integrarsi con le tabelle del database aziendale, recuperando dati utili alle funzionalità.\\
%Il mio lavoro consisteva nel sviluppare la possibilità di effettuare operazioni CRUD tramite gli endpoint dell'API su: richieste di figure professionali, pianificazioni e milestone, oltre a operazioni di sola lettura sulle anagrafiche dell'azienda.\\
%Dopo aver studiato le tecnologie necessarie e svolto alcune esercitazioni, ho iniziato lo sviluppo del prodotto basandomi su due documenti: un documento mock\textsubscript{g} dell'API e un'Analisi dei Requisiti che descriveva il prodotto nella totalità.
